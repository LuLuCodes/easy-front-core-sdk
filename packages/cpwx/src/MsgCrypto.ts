...
import { IApiConfig } from './CPWXCore'
...
export class MsgCrypto {
  ...
  encryptMsg(xmlMsg: string): string {
    ...
    //进行PKCS7补位
    let pack = this.KCS7Encoder(20 + text.length + this.corpId.length)
    //拼接要加密的字符串
    let content = random + buf.toString('binary') + text.toString('binary') + this.corpId + pack
    //实例 AES 加密对象
