...
import { IApiConfig } from './OpenCPWXCore'
...
export class MsgCrypto {
  ...
  constructor(config: MsgCryptoConfig, msgSignature: string, timestamp: string, nonce: string) {
    this.aesModel = 'aes-256-cbc'
    this.token = config.token
    this.receiveId = config.receiveId
  ...
  encryptMsg(xmlMsg: string): string {
    ...
    //进行PKCS7补位
    let pack = this.KCS7Encoder(20 + text.length + this.receiveId.length)
    //拼接要加密的字符串
    let content = random + buf.toString('binary') + text.toString('binary') + this.receiveId + pack
    //实例 AES 加密对象
    ...
    //返回XML结果
    return MsgCrypto.xmlParser.buildObject({
